...
export default function Questions(props){
    ...
    const getQuestions = async () => {
        try{
            const response = await fetch('https://devcon4-back.herokuapp.com/api/questions'); 
    ...
    const handleQuestionSubmit = async (event) => {
        ...
        if (questionFormInputs.category === ''){
            ...
            try{
                ...
                const response = await axios.post('https://devcon4-back.herokuapp.com/api/questions', questionFormInputs);
    ...
    const handleQuestionDelete = async (event) => {
        ...
        console.log(`look here ${event.target.id}`);
            await axios.delete(`https://devcon4-back.herokuapp.com/api/questions/${event.target.id}`)
        } catch(error){
            console.error(error)
        };
        };
    ...
    const handleClickUpdateQuestion = async (event) => {
        event.preventDefault();
        const response = await axios.get(`https://devcon4-back.herokuapp.com/api/questions/${event.target.id}`);
    ...
    const handleQuestionUpdateSubmit = async (event) => {
        event.preventDefault();
        const response = await axios.put(`https://devcon4-back.herokuapp.com/api/questions/${event.target.id}`, {topic: questionUpdate.topic, body: questionUpdate.body});
