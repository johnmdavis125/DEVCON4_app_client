...
export default function ResponseForm(props){
    ...
    const getResponses = async () => {
        try{
            const response = await fetch('https://devcon4-front.herokuapp.com/api/responses'); 
    ...
    const handleResponseSubmit = async (event) => {
        ...
        try{
            const response = await axios.post('https://devcon4-front.herokuapp.com/api/responses', responseFormInputs);
        ...
        const handleResponseDelete = async (event) => {
            ...
            console.log(`look here ${event.target.id}`);
                await axios.delete(`https://devcon4-front.herokuapp.com/api/responses/${event.target.id}`)
            } catch(error){
                console.error(error)
            };
            };
    ...
    const upVote = async (event) => {
        ...
        event.persist(); 
        try {
            const response = await axios.get(`https://devcon4-front.herokuapp.com/api/responses/${event.target.id}`)
            const newVote = await response.data.votes + 1
            
            const resp = await axios.put(`https://devcon4-front.herokuapp.com/api/responses/${event.target.id}`, {votes: newVote});
